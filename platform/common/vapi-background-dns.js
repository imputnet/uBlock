...
// Extracted from Firefox's vapi-background-ext.js.
// When merging upstream changes to that class, apply shared DNS fixes here.
...
export const NetWithDNS = (Base, resolveRecord, skipDNS = ( ) => false) => class extends Base {
        ...
        const promise = resolveRecord(hn, details).then(
            rec => {
                if ( rec === null ) {
                    ...
                    if ( this.dnsDict.get(hn) === promise ) {
                        this.dnsDict.delete(hn);
                    }
                    return;
                }
                return this.dnsToCache(hn, rec, details);
            },
            ( ) => this.dnsToCache(hn)
        );
    ...
    unsuspendAllRequests(discard = false) {
        ...
        super.unsuspendAllRequests(discard);
    }
